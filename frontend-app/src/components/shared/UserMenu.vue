...
<template>
  <div ref="root" class="relative">
    <button
        ...
        @click="toggleUser"
    >
      <img v-if="avatar" :src="avatar" alt="" class="h-9 w-9 rounded-full object-cover" />
      <span v-else class="text-xs select-none">{{ initials }}</span>
    </button>
    ...
    <transition name="menu">
      ...
      >
        <div class="px-4 py-3 border-b border-gray-100 dark:border-zinc-800">
          <p class="text-sm font-semibold text-gray-900 dark:text-zinc-100 truncate">{{ user?.name }}</p>
          <p class="text-xs text-gray-600 dark:text-zinc-400 truncate">{{ user?.email }}</p>
        </div>
