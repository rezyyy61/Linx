    ...
    <div class="mx-auto grid min-h-screen w-full max-w-6xl grid-cols-1 items-stretch gap-8 px-4 py-10 md:grid-cols-2 md:py-16 lg:gap-12">
      <div class="relative hidden md:flex">
        <div class="m-auto">
          ...
          <h1 class="mt-4 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">Set a new password</h1>
      ...
      <div class="flex items-center">
        <div class="relative w-full overflow-hidden rounded-2xl border border-gray-200/70 bg-white/70 p-6 shadow-2xl backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-zinc-800/80 dark:bg-zinc-900/60">
          ...
          <div class="text-center">
            <Icon icon="mdi:lock-reset" class="mx-auto h-10 w-10 text-brand" aria-hidden="true" />
            <h2 class="mt-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Reset password</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-zinc-400">Enter and confirm your new password.</p>
          </div>

          <Transition name="fade">
            <div v-if="alertMsg" class="mt-4 rounded-md bg-blue-50 px-3 py-2 text-sm text-blue-800 ring-1 ring-inset ring-blue-200 dark:bg-blue-950/40 dark:text-blue-200 dark:ring-blue-900" role="status" aria-live="polite">
              {{ alertMsg }}
            </div>
          </Transition>
          ...
          <form class="mt-6 grid gap-5" novalidate @submit.prevent="onSubmit" :aria-busy="loading">
            <div class="grid gap-1.5">
              ...
              <div class="flex items-center justify-between">
                <p v-if="touched.password && !passwordValid" class="error">Use at least 8 characters.</p>
                <p v-else-if="errors.password" class="error">{{ errors.password[0] }}</p>
                <p v-else-if="form.password" class="text-xs text-gray-500 dark:text-zinc-400">Strength:
            ...
            <div class="grid gap-1.5">
              <div class="group relative" @keyup="onKeyEvent" @keydown="onKeyEvent">
                <Icon icon="mdi:lock-check-outline" class="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400 group-focus-within:text-brand" aria-hidden="true" />
                <input
                ...
                <button type="button" :aria-label="show confirm password" :aria-pressed="showConfirm" class="abs-eye" @click="showConfirm = !showConfirm">
                  <Icon :icon="showConfirm ? 'mdi:eye-off-outline' : 'mdi:eye-outline'" class="h-5 w-5" />
                </button>
              </div>
              <p v-if="touched.confirm && !confirmValid" class="error">Passwords don't match.</p>
            ...
            <p class="text-center text-sm text-gray-600 dark:text-zinc-400">
              ...
              <RouterLink to="/auth/forgot-password" class="text-brand underline-offset-2 hover:underline">Request a new link</RouterLink>
            </p>
          </form>
        </div>
      </div>
