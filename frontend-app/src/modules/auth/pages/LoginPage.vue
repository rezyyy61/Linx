      ...
      <div class="relative hidden md:flex">
        <div class="m-auto">
          ...
          <ul class="mt-6 grid max-w-md gap-3 text-sm text-gray-700 dark:text-zinc-300">
            <li class="flex items-start gap-2"><Icon icon="mdi:check-decagram" class="mt-0.5 h-5 w-5 text-brand"/> Passwordless-ready</li>
            <li class="flex items-start gap-2"><Icon icon="mdi:check-decagram" class="mt-0.5 h-5 w-5 text-brand"/> OAuth with Google & Facebook</li>
            <li class="flex items-start gap-2"><Icon icon="mdi:check-decagram" class="mt-0.5 h-5 w-5 text-brand"/> Dark mode perfected</li>
          </ul>

          <div class="mt-8 hidden text-xs text-gray-500 dark:text-zinc-500 md:block">
            By continuing you agree to our
            <RouterLink to="/legal/terms" class="text-brand underline-offset-2 hover:underline">Terms</RouterLink>
            and
            <RouterLink to="/legal/privacy" class="text-brand underline-offset-2 hover:underline">Privacy Policy</RouterLink>.
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center">
        <div
          ...
          <div class="text-center">
            <Icon icon="mdi:shield-lock" class="mx-auto h-10 w-10 text-brand" aria-hidden="true" />
            <h2 class="mt-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Sign in</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-zinc-400">Use your email and password or continue with a provider.</p>
          </div>
          ...
          <div class="mt-6 flex items-center gap-4">
            ...
            <div class="text-xs uppercase tracking-wider text-gray-500 dark:text-zinc-500">or</div>
            <div class="h-px w-full bg-gray-200 dark:bg-zinc-800" />
          </div>
          ...
          <!-- Error summary (a11y) -->
          <Transition name="fade">
            <div v-if="Object.keys(errors).length" class="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-800 ring-1 ring-inset ring-red-200 dark:bg-red-950/30 dark:text-red-200 dark:ring-red-900"
          ...
          <form class="mt-6 grid gap-5" novalidate @submit.prevent="onSubmit" :aria-busy="loading">
            <!-- Email (floating label) -->
            <div class="grid gap-1.5">
              <div class="group relative">
                <Icon icon="mdi:email-outline" class="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400 group-focus-within:text-brand" aria-hidden="true" />
                <input
                    id="email"
                    ref="emailRef"
              ...
              <p id="email-help" v-if="touched.email && !emailValid" class="error">Please enter a valid email address.</p>
              <p v-else-if="errors.email" class="error">{{ errors.email[0] }}</p>
            </div>

            <!-- Password (floating label) -->
            <div class="grid gap-1.5">
              <div class="group relative" @keyup="onKeyEvent" @keydown="onKeyEvent">
                <Icon icon="mdi:lock-outline" class="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400 group-focus-within:text-brand" aria-hidden="true" />
                <input
                    id="password"
              ...
              <div class="flex items-center justify-between">
                <p v-if="touched.password && !passwordValid" class="error">Password must be at least 6 characters.</p>
                <p v-else-if="errors.password" class="error">{{ errors.password[0] }}</p>
                <p v-else-if="form.password" class="text-xs text-gray-500 dark:text-zinc-400">Strength: <span :class="['font-medium', pwdScore >= 3 ? 'text-emerald-600 dark:text-emerald-400' : pwdScore === 2 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400']">{{ pwdLabel }}</span></p>
                <p v-if="capsLockOn" class="text-xs font-medium text-amber-700 dark:text-amber-400">Caps Lock is ON</p>
              </div>
            </div>

            <div class="flex items-center justify-between">
              <!-- Custom switch -->
              <button type="button" role="switch" :aria-checked="form.remember" @click="form.remember = !form.remember"
                ...
                <span :class="['relative inline-flex h-5 w-9 items-center rounded-full transition', form.remember ? 'bg-brand/90' : 'bg-gray-300 dark:bg-zinc-700']">
                  <span :class="['inline-block h-4 w-4 transform rounded-full bg-white shadow transition', form.remember ? 'translate-x-5' : 'translate-x-1']"></span>
                </span>
                <span>Remember me</span>
              </button>

              <RouterLink to="/auth/forgot-password" class="text-sm text-brand underline-offset-2 hover:underline">Forgot password?</RouterLink>
            </div>
            ...
            <p class="text-center text-sm text-gray-600 dark:text-zinc-400">
              ...
              <RouterLink to="/auth/register" class="text-brand underline-offset-2 hover:underline">Create one</RouterLink>
            </p>
          </form>
        </div>
      </div>
