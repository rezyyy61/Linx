<template>
  <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white p-3 dark:border-white/10 dark:bg-zinc-900/70">
    <div class="flex items-center gap-3">
      <div class="h-12 w-12 overflow-hidden rounded-lg border border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
        <img
          v-if="media.coverUrl"
          :src="media.coverUrl"
          alt=""
...
function onTime() {
  ...
  percent.value = Math.min(100, Math.max(0, (current.value / duration.value) * 100))
}
function onSeek() {
  if (!audio.value || !duration.value) return
  audio.value.currentTime = (percent.value / 100) * duration.value
}
function toggle() {
