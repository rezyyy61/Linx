...
const looksLikeCampaign = (p: any) =>
  p?.postable_alias === 'campaign' ||
  (p?.postable_type && String(p.postable_type).includes('Campaign')) ||
  (p?.postable && (p.postable.slug || p.postable.title))

const isCampaignAttachment = computed(() => {
  const p: any = props.post || {}
  return looksLikeCampaign(p) || looksLikeCampaign(fullPost.value || {})
})

const campaignSlug = computed<string | null>(() => {
  const p: any = props.post || {}
...
watchEffect(async () => {
  ...
  if (!looksLikeCampaign(p)) await ensureFullPostOnce()
  if (!isCampaignAttachment.value) return

  if (p?.postable && (p.postable.slug || p.postable.title)) {
    preview.value = normalizeCampaign(p.postable)
    return
  }

  const fp: any = fullPost.value || {}
  if (fp?.postable && (fp.postable.slug || fp.postable.title)) {
    preview.value = normalizeCampaign(fp.postable)
    return
  }

  const slug = campaignSlug.value
  if (!slug) return

  loading.value = true
  try {
    const raw = await fetchCampaignBySlug(slug)
    preview.value = normalizeCampaign(raw)
  } catch {
    preview.value = null
  } finally {
    loading.value = false
