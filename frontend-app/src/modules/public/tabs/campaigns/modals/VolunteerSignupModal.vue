...
defineEmits<{(e:"close"):void,(e:"submit",v:{name:string;email:string;role?:string|null}):void}>()
const name = ref("")
const email = ref("")
const role = ref("")
watch(() => props.open, v => { if (!v) { name.value=""; email.value=""; role.value="" } })
</script>
      ...
      <div class="w-full max-w-md rounded-2xl bg-white dark:bg-neutral-900 p-5 ring-1 ring-black/5 dark:ring-white/10">
        ...
        <div class="mt-5 flex items-center justify-end gap-2">
          ...
          <button
            ...
            :disabled="!name || !email"
            @click="$emit('submit',{name,email,role:role||null})"
          >
            Apply
          </button>
        </div>
