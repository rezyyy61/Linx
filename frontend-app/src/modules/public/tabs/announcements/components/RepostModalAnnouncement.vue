...
import AnnouncementCard, { type AnnouncementCardModel } from "@/modules/public/tabs/announcements/components/AnnouncementCard.vue"
...
const errorMsg = ref("")
const preview = ref<AnnouncementCardModel | null>(null)

function normalizeApiAnnouncement(a:any): AnnouncementCardModel {
  return {
    id: Number(a.id),
...
watchEffect(() => {
  ...
  if (props.initialAnnouncement) {
    preview.value = normalizeApiAnnouncement(props.initialAnnouncement)
    return
  }

  if (props.announcementSlug) {
    api.get(`/public/v1/announcements/${props.announcementSlug}`)
      .then(({ data }) => {
        const payload = data?.data ?? data
        preview.value = normalizeApiAnnouncement(payload)
      })
...
async function onSubmit(){
  ...
  errorMsg.value = ""
  try {
    const idForApi = (preview.value?.id ?? props.announcementId)!
    const r = await createRepostGeneric({
      shareable_alias: "announcement",
