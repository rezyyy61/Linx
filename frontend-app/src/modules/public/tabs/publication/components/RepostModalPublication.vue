...
watchEffect(() => {
  ...
  if (props.initialPublication) {
    preview.value = normalizePublication(props.initialPublication)
    return
  }
  if (props.publicationSlug) {
    api.get(`/public/v1/publications/${encodeURIComponent(props.publicationSlug)}`)
      .then(({data}) => {
        const payload = data?.data ?? data
        preview.value = normalizePublication(payload)
      })
      .catch(() => { preview.value = null })
    return
  }
  if (props.publicationId && !Number.isNaN(Number(props.publicationId))) {
    api.get(`/public/v1/publications/id/${props.publicationId}`)
      .then(({data}) => {
        const payload = data?.data ?? data
        preview.value = normalizePublication(payload)
      })
...
async function onSubmit(){
  ...
  errorMsg.value = ""
  try {
    const idForApi = (preview.value?.id ?? props.publicationId)!
    const r = await createRepostGeneric({
      shareable_alias: "publication",
