...
const status = defineModel<Status>("status", { default: "all" })
...
<template>
  <div class="w-full grid grid-cols-1 md:[grid-template-columns:1fr_auto] items-center gap-3 mb-6">
    <div class="relative h-10 w-full flex justify-start">
      ...
      <input
        ...
        class="h-10 w-[80%] rounded-xl border border-neutral-300 pl-10 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600 dark:border-neutral-700 dark:bg-neutral-800"
        @input="triggerApply(false)"
      >
      <span class="absolute right-[calc(20%+0.5rem)] top-1/2 -translate-y-1/2 text-neutral-400">⌘K</span>
    </div>

    <div class="flex items-center justify-start md:justify-end gap-3 text-sm">
