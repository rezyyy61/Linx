  ...
  async () => {
    ...
    if (props.eventId) {
      for (let i = 0; i < ready.length; i++) {
        await attach(ready[i].id!, FQN_EVENT, props.eventId, "event-document", docs.value.length + i);
...
async function removeDoc(idx: number) {
  ...
  if (!d) return;
  if (props.eventId && d.persisted) {
    try {
      await mediaStore.detachFromModel(d.id, FQN_EVENT, props.eventId, "event-document");
    } catch { /* empty */ }
