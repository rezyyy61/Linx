...
import { listAnnouncements, deleteAnnouncement } from "../api";
import type { Announcement, AnnouncementVisibility } from "../types";

type OrderBy = "publish_at" | "created_at" | "updated_at" | "is_pinned";
type OrderDir = "asc" | "desc";

export function useAnnouncementsList(init?: {
  q?: string;
  ...
  const filters = reactive({
    q: init?.q ?? "",
    visibility: init?.visibility ?? undefined as AnnouncementVisibility | undefined,
    pinned: init?.pinned ?? undefined as boolean | undefined,
  ...
  function setVisibility(v?: AnnouncementVisibility) {
    filters.visibility = v;
    filters.page = 1;
  }

  function setPinned(p?: boolean) {
    filters.pinned = p;
    filters.page = 1;
  }
  ...
  watch(
    () => [filters.q, filters.visibility, filters.pinned, filters.order_by, filters.order_dir, filters.page, filters.per_page],
    () => { fetchList(); },
    { deep: true }
  );
