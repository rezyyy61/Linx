...
import { useAnnouncementMedia } from "@/modules/dashboard/pages/announcements/composables/useAnnouncementMedia";
...
const { docs, addDoc, removeDoc } = useAnnouncementMedia(props.announcementId, {
...
const remainingSlots = computed(() => Math.max(0, 50 - (viewDocs.value.length + activeTasksCount.value)));
...
function iconFor(ext: string) {
  ...
  if (["txt","md","rtf"].includes(e)) return "mdi:file-document-outline";
...
function hydrateFromModel() {
  const base = docs.value.map(d => {
    ...
    return { id: d.id, url: d.url, name, ext: toExt(name), persisted: !!props.announcementId } as DocView;
  });
  viewDocs.value = base;
