...
import { useCampaignMedia } from "@/modules/dashboard/pages/campaigns/composables/useCampaignMedia";
...
const { docs, addDoc, removeDoc } = useCampaignMedia(props.campaignId, {
...
function hydrateFromModel() {
  const base = docs.value.map(d => {
    ...
    return { id: d.id, url: d.url, name, ext: toExt(name), persisted: !!props.campaignId } as DocView;
  });
  viewDocs.value = base;
      ...
      >
        <div class="flex flex-col items-center gap-3">
          ...
          <div class="text-sm font-medium">
            {{ remainingSlots > 0 ? $t("campaign.cover.dropOrClick") : ($t("common.limitReached") || "Limit reached") }}
          </div>
          <div
            v-if="remainingSlots > 0"
            class="text-xs text-gray-500"
          >
            {{ $t("campaign.cover.chooseFile") }} • {{ remainingSlots }}/2
          </div>
          <div
            v-else
            class="text-xs text-gray-500"
