...
const { coverId, coverUrl, setCover, clearCover } = useCampaignMedia(props.campaignId, {
...
const busy = computed(
  () => !!task.value && ["presigning","uploading","finalizing","scanning","processing"].includes(task.value.status)
);
            ...
            <div class="absolute inset-x-0 bottom-0 p-3 md:p-4 bg-gradient-to-t from-black/50 via-black/20 to-transparent flex items-center justify-between">
              <div class="flex items-center gap-2 text-white/90 text-xs md:text-sm">
                ...
                <Icon
                  ...
                  class="w-4 h-4"
                />
                <span v-if="busy">{{ $t("campaign.cover.uploading") }}</span>
