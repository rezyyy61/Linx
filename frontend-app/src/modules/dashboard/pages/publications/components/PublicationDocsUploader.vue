...
const { docs, addDoc, removeDoc } = usePublicationMedia(props.publicationId, {
...
function hydrateFromModel() {
  const base = docs.value.map(d => {
    ...
    return { id: d.id, url: d.url, name, ext: toExt(name), persisted: !!props.publicationId } as DocView;
  });
  viewDocs.value = base;
